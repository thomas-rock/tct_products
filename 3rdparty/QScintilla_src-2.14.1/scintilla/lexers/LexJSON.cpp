...
class LexerJSON : public DefaultLexer {
   ...
   //-----------------------------------------------------------
   static bool IsJSON5IdentifierStart(int ch) {
      return isalpha(ch) || ch == '_' || ch == '$';
   }

   static bool IsJSON5IdentifierContinue(int ch) {
      return isalnum(ch) || ch == '_' || ch == '$';
   }

   static bool AtUnquotedPropertyName(LexAccessor &styler, Sci_Position start) {
      if (!IsJSON5IdentifierStart(styler.SafeGetCharAt(start, '\0'))) {
         return false;
      }
      ...
      while (i < 100) {
         char curr = styler.SafeGetCharAt(start + i, '\0');
         if (!IsJSON5IdentifierContinue(curr)) {
            break;
         }
         i++;
      }

      return IsNextNonWhitespace(styler, start + i - 1, ':');
   }

   static bool IsHexDigitJSON(int ch) {
      return isdigit(ch) ||
            (ch >= 'a' && ch <= 'f') ||
            (ch >= 'A' && ch <= 'F') || ch == '_';
}   //-----------------------------------------------------------
...
void SCI_METHOD LexerJSON::Lex(Sci_PositionU startPos,
   ...
   bool unquotedPropertyName = false;
	while (context.More()) {
		switch (context.state) {
         ...
         case SCE_JSON_PROPERTYNAME:
            if (unquotedPropertyName) {
               if (!IsJSON5IdentifierContinue(context.ch)) {
                  context.SetState(SCE_JSON_DEFAULT);
               }
               break;
            }
...
//         case SCE_JSON_PROPERTYNAME:
		...
		if (context.state == SCE_JSON_DEFAULT) {
         ...
         } else if (IsJSON5IdentifierStart(context.ch) &&
                  AtUnquotedPropertyName(styler, static_cast<Sci_Position>(context.currentPos))) {
            unquotedPropertyName = true;
            context.SetState(SCE_JSON_PROPERTYNAME);			
         ...
         bool hexStart =
            context.ch == '0' &&
            (context.chNext == 'x' || context.chNext == 'X') &&
            (context.chPrev == '+' ||
             context.chPrev == '-' ||
             context.atLineStart ||
             IsASpace(context.chPrev) ||
         ...
         bool hexPrefix =
            (context.ch == 'x' || context.ch == 'X') &&
            context.chPrev == '0';

         bool hexDigit =
            IsHexDigitJSON(context.ch) &&
            (
               IsHexDigitJSON(context.chPrev) ||
               context.chPrev == 'x' ||
               context.chPrev == 'X'
            );
         ...
         bool afterExponent =
            IsADigit(context.ch) && tolower(context.chPrev) == 'e';
